...
function buildWirePath(plane, pointHandler, input)
{
    for (let i = 0, x= 0, y = 0, steps = 0; i < input.length; i++)
    {
        var direction = input[i].substring(0, 1);
        var magnitude = parseInt(input[i].substring(1));
        ...
        for (let step = 0; step < magnitude; step++)
        {
            ...
            pointHandler(plane.getPoint(x, y), steps);
        }
    }
}

let pointHandler1 = function(point,steps)
{
    point.visits = 1;
    if(typeof(point.steps1) == 'undefined')
    {
        point.steps1 = steps;
    }
};
let pointHandler2 = function(point,steps)
{
    point.visits++;
    if(typeof(point.steps2) == 'undefined')
    {
        point.steps2 = steps;
    }
};
...
console.log('3.1: ' + Math.min(...intersections.map(p => Math.abs(p.x) + Math.abs(p.y))));
console.log('3.2: ' + Math.min(...intersections.map(p => p.steps1 + p.steps2)));
